...
function JsonTree({ data }) {
  return (
    <div className="max-h-[60vh] overflow-auto rounded-lg border border-gray-200 p-3 bg-gray-50">
...
function TabDataProcessing() {
  ...
  return (
    <SectionCard
      title="資料處理 (執行: python3 data_csv_convert.py twpas_exam.csv)"
      actions={
        <button
          onClick={run}
          disabled={loading}
    ...
    >
      ...
      {json && (
        <div className="mt-4">
          <h3 className="text-sm font-medium mb-2">output.json</h3>
          <JsonTree data={json} />
        </div>
      )}
    </SectionCard>
...
function TabConverter() {
  ...
  return (
    <SectionCard
      title="Converter (依序執行: node convert_json_via_lib.js pasbundle_config → python3 json_convert.py)"
      actions={
        <button
          onClick={run}
          disabled={loading}
    ...
    >
      ...
      {json && (
        <div className="mt-4">
          <h3 className="text-sm font-medium mb-2">fhir_conversion_outputs.json</h3>
          <JsonTree data={json} />
        </div>
      )}
    </SectionCard>
...
function TabValidator() {
  ...
  const run = async () => {
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        setStatus({ type: "failure", count: data.errorCount || (data.errors?.length ?? 0) });
      }
  ...
  return (
    <SectionCard
      title="Validator (執行: java -jar validator_cli.jar fhir_conversion_outputs.json -version 4.0 -ig tw.gov.mohw.nhi.pas#1.0.7)"
      actions={
        <button
          onClick={run}
          disabled={loading}
    ...
    >
      ...
      {status && (
        <div className="mb-4">
          {status.type === "success" ? (
            <StatusPill color="green" label="Success" />
          ) : (
